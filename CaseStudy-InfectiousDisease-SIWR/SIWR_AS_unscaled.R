...
# - Uses the non-scaled form of the parameters
# - Global measures sample over a ±percent range


# To do's
# - fix the hard-coded tolerence fix for the gradient---currently just manually changing k to be 10x and 
#   then undoing it, so we don't hop over zero when the gradient is calculated...
...
#### Parameters & setup ####
params = c('betaI'=0.256, 'betaW'=1.21, 'xi' = 0.00756, 'k' = 1.1212e-5) # base parameters from Eisenberg 2013
...
gradientfixer = 100 # this is a quick fix---the gradient calculation for the scalar FIMs will sometimes make k 
# negative because it's so small, so we're going to scale it up by 100 for that part of things (and then unscale it in the cost function)
...
FIM.loc.sca = grad(fmapscalar,paramstemp*c(1,1,1,gradientfixer),method="Richardson") %o% grad(fmapscalar,paramstemp*c(1,1,1,gradientfixer),method="Richardson")
...
for(i in 1:numsamples){
  paramsample[i,] = (1+paramsample[i,])*paramstemp
  ...
  FIMsample.sca[[i]] = grad(fmapscalar,paramsample[i,]*c(1,1,1,gradientfixer),method="Richardson") %o% grad(fmapscalar,paramsample[i,]*c(1,1,1,gradientfixer),method="Richardson")
...
for(i in 1:length(eigenvalues)){
  ...
  eigplot = ggplot(df_eig,aes(x,y,xend=x+1,yend=y))+
    ...
    theme_bw(base_size = 14) + labs(x="", y="Eigenvalues")+
  ...
  eigplot
  ggsave(file=paste("eigplot",names[[i]],".pdf",sep=""), width = 1.4, height = 3.25, eigplot)
}
...
#Non-abs-valued eigenvector plots
labels = list("First","Second","Third","Fourth")
...
for(i in 1:length(eigenvalues)){
  for(j in 1:4){
    df_eigvec = data.frame(cbind(1:4,eigenvectors[[i]][,j]))
    names(df_eigvec) = c("x","y")
    eigvec = ggplot(data=df_eigvec,aes(x,y=y,fill=abs(y)))+
      geom_bar(stat = "identity")+
      scale_fill_gradient(low="dodgerblue1", high="firebrick1",space="Lab",limits=c(0, 1), guide =FALSE)+
      theme_bw(base_size = 14) + labs(x="Parameter", y=paste(labels[[j]], "eigenvector"))+
      theme(axis.title.y=element_text(vjust=1.5))+
      theme(panel.border = element_blank(),
            panel.background = element_blank(),
            panel.grid.major = element_blank(),
            ...
            axis.line.x = element_line(color = "black",size=0.4),
            axis.line.y = element_line(color = "black",size=0.4))+
      scale_x_continuous(labels=c('',expression(beta[I]),expression(beta[W]),expression(xi),'k',''))#+
        # don't actually know why it needs extra breaks but it does...
    # scale_y_continuous(limits=c(0,1))
    eigvec
    ggsave(file=paste("eig",j,"load_dir", names[[i]],".pdf",sep=""), width = 3.25, height = 3.25, eigvec)
  }
}
