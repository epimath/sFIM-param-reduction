...
# Update 2-1-18: changed this to work when we've rescaled the parameters to be centered at 0 and between
# -1 and 1. (note edits where we work with delta below)

# This code calculates the sensitivity-based form of the Fisher information matrix (FIM), for use in
...
# Function Output
# - A length(params) x length(params) sized sensitivity FIM.

sFIM = function(tspan,params,x0fcn,xfcn,yfcn,data,delta = 0.001){
  ...
  for (j in 1:length(params)) {
    params1 = params
    params2 = params
    params1[j] = params[j] + delta
    params2[j] = params[j] - delta
    
    x1 = ode(x0fcn(data,params1), tspan, xfcn, params1, method='ode45')
    x2 = ode(x0fcn(data,params2), tspan, xfcn, params2, method='ode45')
    
    X = cbind(X, (yfcn(x1,params1) - yfcn(x2,params2))/(params1[j]-params2[j]))
